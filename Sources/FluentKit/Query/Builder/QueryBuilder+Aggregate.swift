...
extension QueryBuilder {
    ...
    public func count(annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Int> {
        if Model().anyID is any AnyQueryableProperty {
            self.count(\._$id, annotationContext: annotationContext)
        } else if let fieldsIDType = Model.IDValue.self as? any Fields.Type {
            self.aggregate(.count, fieldsIDType.keys.first!, annotationContext: annotationContext)
        } else {
            fatalError("Model '\(Model.self)' has neither @ID nor @CompositeID, this is not valid.")
        }
    }

    public func count<Field>(_ key: KeyPath<Model, Field>, annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Int>
        where Field: QueryableProperty, Field.Model == Model, Field.Value: Sendable
    {
        self.aggregate(.count, key, as: Int.self, annotationContext: annotationContext)
    }
    ...
    public func sum<Field>(_ key: KeyPath<Model, Field>, annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Field.Value?>
        where Field: QueryableProperty, Field.Model == Model, Field.Value: Sendable
    {
        self.aggregate(.sum, key, annotationContext: annotationContext)
    }
    ...
    public func average<Field>(_ key: KeyPath<Model, Field>, annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Field.Value?>
        where Field: QueryableProperty, Field.Model == Model, Field.Value: Sendable
    {
        self.aggregate(.average, key, annotationContext: annotationContext)
    }
    ...
    public func min<Field>(_ key: KeyPath<Model, Field>, annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Field.Value?>
        where Field: QueryableProperty, Field.Model == Model, Field.Value: Sendable
    {
        self.aggregate(.minimum, key, annotationContext: annotationContext)
    }
    ...
    public func max<Field>(_ key: KeyPath<Model, Field>, annotationContext: SQLAnnotationContext?) -> EventLoopFuture<Field.Value?>
        where Field: QueryableProperty, Field.Model == Model, Field.Value: Sendable
    {
        self.aggregate(.maximum, key, annotationContext: annotationContext)
    }
    ...
    {
        self.aggregate(method, Model.path(for: field), as: Result.self, annotationContext: annotationContext)
    }
    ...
    {
        self.aggregate(method, [field], as: Result.self, annotationContext: annotationContext)
    }
        ...
        where Result: Codable & Sendable
    {
        ...
        copy.run(annotationContext: annotationContext) { output in
            do {
