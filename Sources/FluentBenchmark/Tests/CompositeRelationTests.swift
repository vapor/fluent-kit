...
extension FluentBenchmarker {
    ...
    private func testCompositeParent_loading() throws {
        try self.runTest(
            ...
            XCTAssertEqual(
                try child1.$linkedCompositeIdParentModel.get(on: self.database).wait().id, child1.$linkedCompositeIdParentModel.id)
            ...
            XCTAssertEqual(
                try child2.$additionalCompositeIdParentModel.get(on: self.database).wait()?.id, child2.$additionalCompositeIdParentModel.id)
            XCTAssertEqual(
                try child2.$linkedCompositeIdParentModel.get(on: self.database).wait().id, child2.$linkedCompositeIdParentModel.id)
            XCTAssertEqual(
                try child2.$additionalLinkedCompositeIdParentModel.get(on: self.database).wait()?.id,
                child2.$additionalLinkedCompositeIdParentModel.id)

            let child3 = try XCTUnwrap(
                CompositeIDChildModel.query(on: self.database).filter(\.$id == 1).with(\.$compositeIdParentModel).with(
                    \.$additionalCompositeIdParentModel
                ).with(\.$linkedCompositeIdParentModel).with(\.$additionalLinkedCompositeIdParentModel).first().wait())
            let child4 = try XCTUnwrap(
                CompositeIDChildModel.query(on: self.database).filter(\.$id == 2).with(\.$compositeIdParentModel).with(
                    \.$additionalCompositeIdParentModel
                ).with(\.$linkedCompositeIdParentModel).with(\.$additionalLinkedCompositeIdParentModel).first().wait())
    ...
    private func testCompositeChildren_loading() throws {
        try self.runTest(
            ...
            let parent4 = try XCTUnwrap(
                CompositeIDParentModel.query(on: self.database).filter(\.$id.$name == "A").with(\.$compositeIdChildModels).with(
                    \.$additionalCompositeIdChildModels
                ).with(\.$linkedCompositeIdChildModel).with(\.$additionalLinkedCompositeIdChildModel).first().wait())
            let parent5 = try XCTUnwrap(
                CompositeIDParentModel.query(on: self.database).filter(\.$id.$name == "B").with(\.$compositeIdChildModels).with(
                    \.$additionalCompositeIdChildModels
                ).with(\.$linkedCompositeIdChildModel).with(\.$additionalLinkedCompositeIdChildModel).first().wait())
            let parent6 = try XCTUnwrap(
                CompositeIDParentModel.query(on: self.database).filter(\.$id.$name == "C").with(\.$compositeIdChildModels).with(
                    \.$additionalCompositeIdChildModels
                ).with(\.$linkedCompositeIdChildModel).with(\.$additionalLinkedCompositeIdChildModel).first().wait())
    ...
    private func testCompositeParent_nestedInCompositeID() throws {
        try self.runTest(
            ...
            let parentFirstAgain = try XCTUnwrap(
                CompositeParentTheFirst.query(on: self.database).filter(\.$id.$parent.$id == anyGalaxy.requireID()).with(\.$id.$parent)
                    .with(\.$children).first().wait())
...
final class CompositeIDChildModel: Model, @unchecked Sendable {
    ...
    struct ModelMigration: Migration {
        func prepare(on database: any Database) -> EventLoopFuture<Void> {
            database.schema(CompositeIDChildModel.schema)
                ...
                .foreignKey(
                    ["comp_parent_model_name", "comp_parent_model_dimensions"],
                ...
                .foreignKey(
                    ["addl_comp_parent_model_name", "addl_comp_parent_model_dimensions"],
                    references: CompositeIDParentModel.schema, ["name", "dimensions"]
                )
                ...
                .foreignKey(
                    ["comp_linked_model_name", "comp_linked_model_dimensions"],
                    references: CompositeIDParentModel.schema, ["name", "dimensions"]
                )
                ...
                .foreignKey(
                    ["addl_comp_linked_model_name", "addl_comp_linked_model_dimensions"],
                    references: CompositeIDParentModel.schema, ["name", "dimensions"]
                )
                .create()
        }
    ...
    struct ModelSeed: Migration {
        func prepare(on database: any Database) -> EventLoopFuture<Void> {
            [
                CompositeIDChildModel(
                    id: 1, parentId: .init(name: "A"), additionalParentId: nil, linkedId: .init(name: "A"), additionalLinkedId: nil),
                CompositeIDChildModel(
                    id: 2, parentId: .init(name: "A"), additionalParentId: .init(name: "B"), linkedId: .init(name: "B"),
                    additionalLinkedId: .init(name: "A")),
                CompositeIDChildModel(
                    id: 3, parentId: .init(name: "A"), additionalParentId: .init(name: "B"), linkedId: .init(name: "C"),
                    additionalLinkedId: .init(name: "B")),
            ].create(on: database)
        }
...
extension DatabaseSchema.Constraint {
    ...
    static func optionalCompositeReferenceCheck(_ field1: FieldKey, _ field2: FieldKey, _ moreFields: FieldKey...)
            ...
            .check(
                SQLGroupExpression(
                    SQLBinaryExpression(
                        ...
                        SQLGroupExpression(
                            SQLBinaryExpression(
                                SQLFunction.coalesce([SQLLiteral.null] + fields.dropFirst().map { $0 }), .is, SQLLiteral.null))
                    ))))
    }
}
    ...
    struct ModelMigration: Migration {
        func prepare(on database: any Database) -> EventLoopFuture<Void> {
            database.schema(CompositeParentTheSecond.schema)
                ...
                .foreignKey(
                    "ref_parent_id", references: CompositeParentTheFirst.schema, "parent_id", onDelete: .cascade, onUpdate: .cascade
                )
