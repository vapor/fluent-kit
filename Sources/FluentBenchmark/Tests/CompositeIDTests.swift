...
extension FluentBenchmarker {
    ...
    private func testCompositeID_update() throws {
        try self.runTest(
            ...
            XCTAssertEqual(
                try CompositeIDModel.query(on: self.database).filter(\.$additionalInfo == "additional").count(\.$id.$name).wait(), 1)

            try CompositeIDModel.query(on: self.database).filter(\.$id.$name == "A").filter(\.$id.$dimensions == 1).set(
                \.$id.$dimensions, to: 3
            ).update().wait()
    ...
    private func testCompositeID_asPivot() throws {
        try self.runTest(
            ...
            XCTAssertTrue(
                pivots.contains(where: {
                    $0.id!.$galaxy.id == milkyWayGalaxy.id! && $0.id!.$jurisdiction.id == oldJurisdiction.id! && $0.id!.rank == 1
                }))
            XCTAssertTrue(
                pivots.contains(where: {
                    $0.id!.$galaxy.id == milkyWayGalaxy.id! && $0.id!.$jurisdiction.id == noneJurisdiction.id! && $0.id!.rank == 2
                }))
            XCTAssertTrue(
                pivots.contains(where: {
                    $0.id!.$galaxy.id == andromedaGalaxy.id! && $0.id!.$jurisdiction.id == noneJurisdiction.id! && $0.id!.rank == 0
                }))
        }
    }
    ...
    private func testCompositeID_eagerLoaders() throws {
        try self.runTest(
            ...
            let milkyWayGalaxy = try XCTUnwrap(
                Galaxy.query(on: self.database).filter(\.$name == "Milky Way").with(\.$jurisdictions).first().wait())
            XCTAssertEqual(milkyWayGalaxy.jurisdictions.count, 2)

            let militaryJurisdiction = try XCTUnwrap(
                Jurisdiction.query(on: self.database).filter(\.$title == "Military").with(\.$galaxies).with(\.$galaxies.$pivots).first()
                    .wait())
            XCTAssertEqual(militaryJurisdiction.galaxies.count, 2)
            XCTAssertEqual(militaryJurisdiction.$galaxies.pivots.count, 2)

            let corporateMilkyWayPivot = try XCTUnwrap(
                GalacticJurisdiction.query(on: self.database)
    ...
    private func testCompositeID_arrayCreateAndDelete() throws {
        try self.runTest(
            ...
            let milkyWayGalaxyReloaded = try XCTUnwrap(
                Galaxy.query(on: self.database).filter(\.$name == "Milky Way").with(\.$jurisdictions).with(\.$jurisdictions.$pivots).first()
                    .wait())
            ...
            let milkyWayGalaxyRevolutions = try XCTUnwrap(
                Galaxy.query(on: self.database).filter(\.$name == "Milky Way").with(\.$jurisdictions).first().wait())
