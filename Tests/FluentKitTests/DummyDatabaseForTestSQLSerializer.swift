...
struct FakedDatabaseRow: DatabaseOutput, SQLRow {
    ...
    func decode<D: Decodable>(column c: String, as: D.Type) throws -> D {
        guard case .some(.some(let v)) = self.data[c] else { throw DecodingError.keyNotFound(BasicCodingKey.key(c), .init(codingPath: [], debugDescription: "")) }
...
struct DummyDatabaseConverterDelegate: SQLConverterDelegate {
    func customDataType(_ dataType: DatabaseSchema.DataType) -> (any SQLExpression)? {
        switch dataType {
        case .uuid:
            return SQLUnsafeRaw("UUID")
        case .bool:
            return SQLUnsafeRaw("BOOL")
        case .data:
            return SQLUnsafeRaw("BYTEA")
        case .datetime:
            return SQLUnsafeRaw("TIMESTAMPTZ")
        default:
            return nil
        }
    }
