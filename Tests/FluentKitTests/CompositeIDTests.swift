...
final class CompositeIDTests: XCTestCase {
    ...
    func testCompositeModelCRUD() throws {
        ...
        XCTAssertEqual(
            ...
            #"INSERT INTO "composite+planet+tag" ("planet_id", "tag_id", "notation", "createdAt", "updatedAt", "deletedAt") VALUES ($1, $2, $3, $4, $5, DEFAULT)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(1).sql,
            #"UPDATE "composite+planet+tag" SET "notation" = $1, "updatedAt" = $2 WHERE ("composite+planet+tag"."planet_id" = $3 AND "composite+planet+tag"."tag_id" = $4) AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $5)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(2).sql,
            #"UPDATE "composite+planet+tag" SET "planet_id" = $1, "updatedAt" = $2 WHERE ("composite+planet+tag"."planet_id" = $3 AND "composite+planet+tag"."tag_id" = $4) AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $5)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(3).sql,
            #"UPDATE "composite+planet+tag" SET "updatedAt" = $1, "deletedAt" = $2 WHERE ("composite+planet+tag"."planet_id" = $3 AND "composite+planet+tag"."tag_id" = $4) AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $5)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(4).sql,
            #"UPDATE "composite+planet+tag" SET "updatedAt" = $1, "deletedAt" = NULL WHERE ("composite+planet+tag"."planet_id" = $2 AND "composite+planet+tag"."tag_id" = $3)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(5).sql,
            #"DELETE FROM "composite+planet+tag" WHERE ("composite+planet+tag"."planet_id" = $1 AND "composite+planet+tag"."tag_id" = $2)"#)
    }
    ...
    func testCompositeModelQuery() throws {
        ...
        _ = try CompositePlanetTag.query(on: db).filter(\.$id.$planet.$id == model.id!.$planet.id).filter(
            \.$id.$tag.$id == model.id!.$tag.id
        ).all().wait()
    ...
    func testCompositeIDRelations() throws {
        ...
        let planet = PlanetUsingCompositePivot(
            id: model.$id.$planet.id, name: "Planet", starId: .init(uuidString: "22222222-2222-2222-2222-222222222222")!)
        ...
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(0).sql,
            #"SELECT "planets"."id" AS "planets_id", "planets"."name" AS "planets_name", "planets"."star_id" AS "planets_star_id" FROM "planets" WHERE "planets"."id" = $1 LIMIT 1"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(1).sql,
            #"SELECT "composite+planet+tag"."planet_id" AS "composite+planet+tag_planet_id", "composite+planet+tag"."tag_id" AS "composite+planet+tag_tag_id", "composite+planet+tag"."notation" AS "composite+planet+tag_notation", "composite+planet+tag"."createdAt" AS "composite+planet+tag_createdAt", "composite+planet+tag"."updatedAt" AS "composite+planet+tag_updatedAt", "composite+planet+tag"."deletedAt" AS "composite+planet+tag_deletedAt" FROM "composite+planet+tag" WHERE "composite+planet+tag"."planet_id" = $1 AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $2)"#
        ...
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(5).sql,
            #"SELECT COUNT("composite+planet+tag"."planet_id") AS "aggregate" FROM "composite+planet+tag" WHERE "composite+planet+tag"."planet_id" = $1 AND "composite+planet+tag"."tag_id" = $2 AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $3)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(6).sql,
            #"SELECT COUNT("composite+planet+tag"."planet_id") AS "aggregate" FROM "composite+planet+tag" WHERE "composite+planet+tag"."planet_id" = $1 AND "composite+planet+tag"."tag_id" = $2 AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $3)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(7).sql,
            #"UPDATE "composite+planet+tag" SET "updatedAt" = $1, "deletedAt" = $2 WHERE "composite+planet+tag"."planet_id" = $3 AND "composite+planet+tag"."tag_id" = $4 AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $5)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(8).sql,
            #"UPDATE "composite+planet+tag" SET "updatedAt" = $1, "deletedAt" = $2 WHERE "composite+planet+tag"."planet_id" = $3 AND ("composite+planet+tag"."deletedAt" IS NULL OR "composite+planet+tag"."deletedAt" > $4)"#
        )
    }
    ...
    func testSiblingAttachWithAsyncEditClosures() async throws {
        ...
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(1).sql,
            #"SELECT "planets"."id" AS "planets_id", "planets"."name" AS "planets_name", "planets"."star_id" AS "planets_star_id", "planets"."possible_star_id" AS "planets_possible_star_id", "planets"."deleted_at" AS "planets_deleted_at" FROM "planets" WHERE ("planets"."deleted_at" IS NULL OR "planets"."deleted_at" > $1)"#
        )
        XCTAssertEqual(
            try db.sqlSerializers.xctAt(2).sql,
            #"INSERT INTO "composite+planet+tag" ("planet_id", "tag_id", "notation", "createdAt", "updatedAt", "deletedAt") VALUES ($1, $2, $3, $4, $5, DEFAULT)"#
    ...
    func testCompositeIDFilterByID() throws {
        ...
        _ = try CompositePlanetTag.query(on: db).filter(\.$id == CompositePlanetTag.IDValue.init(planetID: planetId, tagID: tagId)).all()
            .wait()
        _ = try CompositePlanetTag.query(on: db).filter(\.$id != CompositePlanetTag.IDValue.init(planetID: planetId, tagID: tagId)).all()
            .wait()
    ...
    func testCompositeParentAndChildQuerying() throws {
        ...
        _ = try CompositeMoon(
            name: "", planetSolarSystemId: systemId, planetNormalizedOrdinal: 1,
            progenitorId: .init(solarSystemId: systemId, normalizedOrdinal: 2)
        ).$progenitor.query(on: db).all().wait()
        _ = try CompositeMoon(name: "", planetSolarSystemId: systemId, planetNormalizedOrdinal: 1, progenitorId: nil).$progenitor.query(
            on: db
        ).all().wait()
        _ = try CompositeMoon(
            name: "", planetSolarSystemId: systemId, planetNormalizedOrdinal: 1, progenitorId: nil,
            planetoidId: .init(solarSystemId: systemId, normalizedOrdinal: 3)
        ).$planetoid.query(on: db).all().wait()
        _ = try CompositeMoon(name: "", planetSolarSystemId: systemId, planetNormalizedOrdinal: 1, progenitorId: nil, planetoidId: nil)
            .$planetoid.query(on: db).all().wait()
        ...
        let allPlanetFields =
            #""composite+planet"."system_id" AS "composite+planet_system_id", "composite+planet"."nrm_ord" AS "composite+planet_nrm_ord", "composite+planet"."name" AS "composite+planet_name" FROM "composite+planet""#
        let allMoonFields =
            #""composite+moon"."id" AS "composite+moon_id", "composite+moon"."name" AS "composite+moon_name", "composite+moon"."planet_system_id" AS "composite+moon_planet_system_id", "composite+moon"."planet_nrm_ord" AS "composite+moon_planet_nrm_ord", "composite+moon"."progenitorSystem_id" AS "composite+moon_progenitorSystem_id", "composite+moon"."progenitorNrm_ord" AS "composite+moon_progenitorNrm_ord", "composite+moon"."planetoid_system_id" AS "composite+moon_planetoid_system_id", "composite+moon"."planetoid_nrm_ord" AS "composite+moon_planetoid_nrm_ord" FROM "composite+moon""#

        let expectedQueries: [(String, [any Encodable])] = [
            (
                #"SELECT \#(allPlanetFields) WHERE ("composite+planet"."system_id" = $1 AND "composite+planet"."nrm_ord" = $2)"#,
                [systemId, 1]
            ),
            (
                #"SELECT \#(allPlanetFields) WHERE ("composite+planet"."system_id" = $1 AND "composite+planet"."nrm_ord" = $2)"#,
            ...
            (#"SELECT \#(allPlanetFields) WHERE ("composite+planet"."system_id" IS NULL AND "composite+planet"."nrm_ord" IS NULL)"#, []),
            (
                #"SELECT \#(allMoonFields) WHERE ("composite+moon"."planet_system_id" = $1 AND "composite+moon"."planet_nrm_ord" = $2)"#,
                [systemId, 1]
            ),
            (
                #"SELECT \#(allMoonFields) WHERE ("composite+moon"."progenitorSystem_id" = $1 AND "composite+moon"."progenitorNrm_ord" = $2)"#,
                [systemId, 2]
            ),
            (
                #"SELECT \#(allMoonFields) WHERE ("composite+moon"."planetoid_system_id" = $1 AND "composite+moon"."planetoid_nrm_ord" = $2)"#,
                [systemId, 3]
            ),
        ]
    ...
    func testCompositeParentChildMutating() throws {
        let db = DummyDatabaseForTestSQLSerializer()
        let sysId = UUID(uuidString: "00000000-0000-0000-0000-000000000000")!
        let sys2Id = UUID(uuidString: "11111111-1111-1111-1111-111111111111")!
        ...
        let moon2 = CompositeMoon(
            name: "C", planetSolarSystemId: sysId, planetNormalizedOrdinal: 1,
            progenitorId: .init(solarSystemId: sysId, normalizedOrdinal: 2))
        let moon3 = CompositeMoon(name: "D", planetSolarSystemId: sysId, planetNormalizedOrdinal: 1)
        let moon4 = CompositeMoon(
            name: "E", planetSolarSystemId: sysId, planetNormalizedOrdinal: 1,
            planetoidId: .init(solarSystemId: sysId, normalizedOrdinal: 3))

        db.fakedRows.append(contentsOf: [
            [.init(["id": UUID()])], [.init(["id": UUID()])], [.init(["id": UUID()])], [.init(["id": UUID()])], [.init(["id": UUID()])],
        ])
        ...
        let moonCols =
            #""id", "name", "planet_system_id", "planet_nrm_ord", "progenitorSystem_id", "progenitorNrm_ord", "planetoid_system_id", "planetoid_nrm_ord""#
        let fourVals = "$1, $2, $3, $4, NULL, NULL, NULL, NULL"
        let sixVals1 = "$1, $2, $3, $4, $5, $6, NULL, NULL"
        let sixVals2 = "$1, $2, $3, $4, NULL, NULL, $5, $6"
        let expectedQueries: [(String, [any Encodable], UInt)] = [
            ...
            (
                #"UPDATE "composite+planet" SET "name" = $1 WHERE ("composite+planet"."system_id" = $2 AND "composite+planet"."nrm_ord" = $3)"#,
                ["AA", sysId, 1], #line
            ),
            (
                #"UPDATE "composite+moon" SET "planet_system_id" = $1, "planet_nrm_ord" = $2 WHERE "composite+moon"."id" = $3"#,
                [sys2Id, 2, moon1.id!], #line
            ),
            (
                #"UPDATE "composite+moon" SET "progenitorSystem_id" = NULL, "progenitorNrm_ord" = NULL WHERE "composite+moon"."id" = $1"#,
                [moon2.id!], #line
            ),
            (
                #"UPDATE "composite+moon" SET "planetoid_system_id" = $1, "planetoid_nrm_ord" = $2 WHERE "composite+moon"."id" = $3"#,
                [sys2Id, 3, moon3.id!], #line
            ),
            (
                #"UPDATE "composite+moon" SET "planetoid_system_id" = NULL, "planetoid_nrm_ord" = NULL WHERE "composite+moon"."id" = $1"#,
                [moon4.id!], #line
            ),
        ]
    ...
    func testCompositeParentChildEncoding() throws {
        ...
        let moonJDat =
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":\#(sysJId)},"planetoid":{"id":\#(sysJId)},"progenitor":{"id":\#(sysJId)}}"#
        ...
        XCTAssertEqual(
            try jsonString(moon),
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":\#(sysJId)},"planetoid":{"id":null},"progenitor":{"id":null}}"#)
        ...
        XCTAssertEqual(
            try jsonString(planet),
            #"{"fragment":\#(moonJDat),"id":\#(sysJId),"moons":[\#(moonJDat)],"moonsMade":[\#(moonJDat)],"name":"A"}"#)
        ...
        XCTAssertEqual(
            try jsonString(moon),
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":\#(sysJId),"name":"A"},"planetoid":{"id":\#(sysJId),"name":"A"},"progenitor":{"id":\#(sysJId),"name":"A"}}"#
        )
    }

    func testCompositeParentChildDecoding() throws {
        let decoder = JSONDecoder()
        func unjsonString<D: Decodable>(_ json: String, as: D.Type = D.self) throws -> D {
            try decoder.decode(D.self, from: json.data(using: .utf8)!)
        }

        let sysId = UUID(uuidString: "00000000-0000-0000-0000-000000000000")!
        let moonId = UUID()
        let sysJId = #""\#(sysId.uuidString)""#
        let moonJId = #""\#(moonId.uuidString)""#

        let planet1 = try unjsonString(
            #"{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}},"name":"A"}"#, as: CompositePlanet.self)
        ...
        let moon1 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"planetoid":{"id":null},"progenitor":{"id":null}}"#,
            as: CompositeMoon.self)
        ...
        let moon1_1 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}},"name":"A"},"planetoid":{"id":null},"progenitor":{"id":null}}"#,
            as: CompositeMoon.self)
        XCTAssertNil(moon1_1.$orbiting.value)

        let moon2 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"planetoid":{"id":null},"progenitor":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}}}"#,
            as: CompositeMoon.self)
        ...
        let moon2_1 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"planetoid":{"id":null},"progenitor":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}},"name":"A"}}"#,
            as: CompositeMoon.self)
        XCTAssertNilNil(moon2_1.$progenitor.value)

        let moon3 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"planetoid":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"progenitor":{"id":null}}"#,
            as: CompositeMoon.self)
        ...
        let moon3_1 = try unjsonString(
            #"{"id":\#(moonJId),"name":"B","orbiting":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}}},"planetoid":{"id":{"normalizedOrdinal":1,"solarSystem":{"id":\#(sysJId)}},"name":"A"},"progenitor":{"id":null}}"#,
            as: CompositeMoon.self)
...
private func XCTAssertNilNil<V>(
    _ expression: @autoclosure () throws -> V??, _ message: @autoclosure () -> String = "", file: StaticString = #filePath,
    line: UInt = #line
) {
    func messageIfGiven() -> String {
        let m = message()
    ...
    case .success(.some(.some(let value))):
        return XCTFail("XCTAssertNilNil failed: \".some(.some(\(value)))\"\(messageIfGiven())", file: file, line: line)
...
final class CompositePlanet: Model, @unchecked Sendable {
    ...
    final class IDValue: Fields, Hashable, @unchecked Sendable {
        ...
        static func == (lhs: IDValue, rhs: IDValue) -> Bool {
            lhs.$solarSystem.id == rhs.$solarSystem.id && lhs.normalizedOrdinal == rhs.normalizedOrdinal
        }
        func hash(into hasher: inout Hasher) {
            hasher.combine(self.$solarSystem.id)
            hasher.combine(self.normalizedOrdinal)
        }
    }
...
final class CompositeMoon: Model, @unchecked Sendable {
    ...
    init() {}
    init(
        id: UUID? = nil, name: String, planetSolarSystemId: SolarSystem.IDValue, planetNormalizedOrdinal: Int,
        progenitorId: CompositePlanet.IDValue? = nil, planetoidId: CompositePlanet.IDValue? = nil
    ) {
        if let id = id { self.id = id }
        self.name = name
